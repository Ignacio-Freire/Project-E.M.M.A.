...
class MealPrep:
    ...
    def __log(self, message):
        ...
        if self.verbose.upper() == 'YES':
            print('[{}] Emma, as a chef, says: {}'.format(strftime("%H:%M:%S", localtime()), message))
    ...
    def create_recipes(self, cant):
        ...
        for i in range(cant):
            main = ['        {}'.format(random.choice(protein))]
            side_one = ['        {}'.format(random.choice(sides))]
            side_two = ['        {}'.format(random.choice(sides))]
            main_season = ['        {}'.format(random.choice(main_seasoning))]
            sec_season = ['        {}'.format(random.choice(secondary_seasonings))]
    ...
    def grocery_list(self, recipes_list):
        ...
        tobuy = ['Butcher:']
        for _i in protein:
            cant = recipes_list.count(str('        '+_i))
